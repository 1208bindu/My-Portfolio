...
exports.getResumes = async (req, res, next) => {
  try {
    ...
    if (!resumes.length)
      return res
        .status(404)
        .json({ success: false, error: "There is no resumes to view" });
...
exports.AddResumes = async (req, res, next) => {
  try {
    ...
    if (!file)
      return res
        .status(400)
        .json({ success: false, error: "Please Upload a PDF File" });
    ...
    if (err.name == "ValidationError")
      return res
        .status(422)
        .json({ success: false, error: err.errors.name.message });
    return res
      .status(500)
      .json({ success: false, error: "Server Error : " + err });
  }
...
exports.deleteResume = async (req, res, next) => {
  try {
    ...
    return res
      .status(200)
      .json({ success: true, msg: "File Has Been Deleted Successfly" });
  } catch (err) {
    return res
      .status(500)
      .json({ success: false, error: "Server Error : " + err });
